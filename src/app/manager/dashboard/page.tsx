...
export default function Page() {
  const { data, isPending } = useQuery<DashboardData>({
    ...
    queryFn: () => fetch("/api/manager/dashboard").then((r) => r.json()),
    ...
    <div className="p-4 sm:p-6 flex flex-col flex-1 h-full w-full gap-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <KpiCard
          ...
          value={
            kpis !== undefined ? formatUSD(kpis.revenueThisMonthCents) : "—"
          }
          icon={<DollarSignIcon className="h-4 w-4" />}
          loading={isPending}
          highlight={
            kpis && kpis.revenueThisMonthCents > 0 ? "success" : undefined
          }
        />
        <KpiCard
