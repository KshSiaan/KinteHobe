...
import type { SSLIPNRequest, SSLVerifyResponse } from "../type";
import { order, transaction } from "@/db/schema/order-schema";
import { eq } from "drizzle-orm";
import { createNotification } from "@/lib/notifications";
import * as Sentry from "@sentry/nextjs";
import { fraud } from "@/db/schema/fraud-schema";
...
async function verificationFailed(
  ...
  return Response.json({ message: FAILURE_MESSAGE }, { status: 400 });
}
...
export async function POST(request: Request) {
  ...
  if (!orderRecord) {
    return Response.json({ message: "Order not found" }, { status: 401 });
  }
  ...
  if (
    ...
    ipnData.currency !== "BDT"
  ) {
    return verificationFailed(orderId, userId, "ORDER_VERIFICATION_FAILED");
  }
  ...
  if (!transactionRecord) {
    return verificationFailed(orderId, userId, "TRANSACTION_NOT_FOUND");
  }

  // Validate risk level
  if (String(ipnData.risk_level) === "1") {
    Sentry.logger.info("IPN Data recieved: ", {
      data: ipnData,
      risk: ipnData.risk_level,
      riskType: typeof ipnData.risk_level,
      isEqual: String(ipnData.risk_level) === "1",
    });
    ...
    const [fraudRecord] = await db
      ...
      .returning({ id: fraud.id });

    if (!fraudRecord) {
      return verificationFailed(
        orderId,
        userId,
        "FRAUD_RECORD_CREATION_FAILED",
      );
    }
  }
  ...
  if (!verifyPaymentRequest.ok) {
    return verificationFailed(orderId, userId, "PAYMENT_VERIFICATION_FAILED");
  }
  ...
  if (!updatedOrder) {
    return verificationFailed(orderId, userId, "ORDER_UPDATE_FAILED");
  }
  ...
  if (!updatedTransaction) {
    return verificationFailed(orderId, userId, "TRANSACTION_UPDATE_FAILED");
  }
