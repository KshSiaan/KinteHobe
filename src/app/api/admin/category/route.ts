...
export async function POST(request: Request) {
  ...
  const [
    { data: createdIcon, error: iconError },
    { data: createdBanner, error: bannerError },
  ] = await Promise.all([
  ...
  if (!data || data.length === 0) {
    const { error } = await getSupabaseStorageClient()
      .storage.from("category")
      .remove([iconPath, bannerPath]);

    return Response.json(
      {
        message:
          error?.message || "Failed to create category, and cleanup failed",
      },
      { status: 500 },
    );
  }
