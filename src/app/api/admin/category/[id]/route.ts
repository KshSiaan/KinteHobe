...
export async function PUT(
  ...
  if (image instanceof File) {
    try {
      ...
      const { error: iconError } = await uploadOptimizedImage(
        image,
        newIconPath,
        {
  ...
  if (banner instanceof File) {
    try {
      ...
      const { error: bannerError } = await uploadOptimizedImage(
        banner,
        newBannerPath,
        {
      ...
      return Response.json(
        {
          message:
            error instanceof Error ? error.message : "Failed to process banner",
        },
        { status: 500 },
      );
    }
