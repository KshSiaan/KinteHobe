...
export async function GET(request: Request) {
  ...
  const conditions = [
    status
      ? eq(
          fraud.current_status,
          status as "pending" | "reviewed" | "resolved" | "rejected",
        )
      : undefined,
    search ? sql`${fraud.id}::text ILIKE ${`%${search}%`}` : undefined,
  ];
  ...
  const data = await db
    ...
    .from(fraud)
    .leftJoin(transaction, eq(fraud.transactionId, transaction.id))
  ...
  const totalResult = await db.select({ count: db.$count(fraud) }).from(fraud);
