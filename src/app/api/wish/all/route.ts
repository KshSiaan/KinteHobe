...
export async function GET(request: Request) {
  ...
  const wishedProducts = await db
    .select()
    .from(wishlist)
    .where(eq(wishlist.userId, user.session.userId))
  ...
  return new Response(
    JSON.stringify({
      products: wishedProducts.map((wish) => ({
        ...wish.product,
        base: {
          ...wish.product_variant,
          image: toProductPublicUrl(wish.product_variant.images[0]),
        },
      })),
    }),
    {
