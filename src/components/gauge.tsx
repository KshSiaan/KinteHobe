...
export function Gauge({
  ...
  const getColor = (colorProp: typeof primary, isSecondary = false) => {
    ...
    if (typeof colorProp === "string") {
      return (
        defaultColors[colorProp as keyof typeof defaultColors] || colorProp
      );
    }
  ...
  const generateTickMarks = () => {
    ...
    for (let i = 0; i <= tickCount; i++) {
      ...
      const x2 =
        circleSize / 2 + tickRadius * Math.cos((angle * Math.PI) / 180);
      const y2 =
        circleSize / 2 + tickRadius * Math.sin((angle * Math.PI) / 180);
    ...
    <div className="relative inline-block">
      ...
      >
        ...
        {thresholds?.map((threshold, index) => {
          ...
          const x =
            circleSize / 2 +
            indicatorRadius * Math.cos((angle * Math.PI) / 180);
          const y =
            circleSize / 2 +
            indicatorRadius * Math.sin((angle * Math.PI) / 180);
